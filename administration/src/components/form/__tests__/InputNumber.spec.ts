...
describe('InputNumber.vue', () => {
  ...
  beforeEach(() => {
    component = shallowMount(InputNumber, {
      propsData: {
        id: 'testId'
      }
    });
  ...
  it('check default values of the text input', () => {
    ...
    expect(component.props().max).toBeNull();
    expect(component.props().min).toBeNull();
    ...
    expect(component.props().step).toBeNull();
  ...
  it('check if event is emitted when the input value changes', async () => {
    component.find('input').setValue(15);
  ...
  it('check if the attribute type of the input field is text', () => {
    expect(component.find('input').attributes().type).toBe('number');
  });
